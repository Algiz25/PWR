	...
	public static void main(String[] args) {
		...
		while(!halt) {
			...
			if (word[0].equalsIgnoreCase("lq") && word.length==1) {
				System.out.println(doc[currentDocNo].link.findLongestSequence());
				continue;
			}
			...
			if(word[0].equalsIgnoreCase("sa") && word.length==1) {
				doc[currentDocNo].showArray(doc[currentDocNo].getWeights());
				continue;
			}
