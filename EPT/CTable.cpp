...
CTable *CTable::pcClone() {
    CTable *copy = new CTable(*this); //in this way we can reuse the code
    return copy;
}


void v_mod_tab(CTable *pcTab, int iNewSize) { //this one will change the pcTab
    pcTab->bSetNewSize(iNewSize);
}

void v_mod_tab(CTable cTab, int iNewSize) { //this one creates a copy
    cTab.bSetNewSize(iNewSize);
}

void print_tab(const CTable &cTab) {
    for (int i = 0; i < cTab.i_table_len; i++) {
        cout << cTab.i_table[i] << " ";
    }
    cout << endl;
}
