...
void CTable::setTabVal(int iIndex, int iValue) {
    i_table[iIndex] = iValue;
}

int CTable::getTabVal(int iIndex) {
    return i_table[iIndex];
}

int CTable::getTabLen() {
    return i_table_len;
}
...
void print_tab(CTable &cTab) {
    for (int i = 0; i < cTab.getTabLen(); i++) {
        cout << cTab.getTabVal(i) << " ";
    }
    cout << endl;
}
