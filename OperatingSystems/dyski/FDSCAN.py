...
class FDSCAN:
    ...
    def simulate(self):
        ...
        while completed < len(self.requests):
            ...
            if is_moving and current_request:
                ...
                # find closest
                closest = min(available, key=lambda req: abs(req.deadline_time - self.curr_time - abs(req.position - self.head_position)))
    ...
    def print_results(self):
        ...
        plt.title("Ruch głowicy - Feasible Deadline SCAN", fontsize=14).set_color(color='white')
...
if __name__ == "__main__":
    ...
    requests = [DR.Request(pos, time, deadline) for pos, time, deadline in zip(request_positions, request_times, request_deadlines)]

    head_start = 50

    fd_scan = FDSCAN(requests, head_start)
    fd_scan.simulate()
